...
def read_in_data_from_wrds():
  ...
  DirectorsUS = pd.DataFrame(data=(db.raw_sql(f"SELECT TICKER, CLASSIFICATION, NUM_OF_SHARES, EMPLOYMENT_CEO, EMPLOYMENT_CHAIRMAN, AUDIT_MEMBERSHIP, CG_MEMBERSHIP, COMP_MEMBERSHIP, NOM_MEMBERSHIP, OWNLESS1, PCNT_CTRL_VOTINGPOWER FROM risk.rmdirectors WHERE YEAR BETWEEN '{lastyear}' AND '{thisyear}' AND TICKER IN {SP500List}")))
  count_committees(DirectorsUS)
  Governance = pd.DataFrame(data=(db.raw_sql(f"SELECT TICKER, DUALCLASS FROM risk.rmgovernance WHERE YEAR BETWEEN '{lastyear}' AND '{thisyear}' AND TICKER IN {SP500List}")))
  print(Governance)
  Execucomp = pd.DataFrame(data=(db.raw_sql(f"SELECT TICKER, NUMMTGS FROM comp_execucomp.codirfin WHERE YEAR BETWEEN '{lastyear}' AND '{thisyear}' AND TICKER IN {SP500List}")))
