...
class myData:
  ...
  def __init__(self):
    self.SP500List, self.today_date, self.year_ago_date, self.thisyear, self.lastyear, self.db = None, None, None, None, None, None
  ...
  def count_committees(self):
    dataframe = pd.DataFrame(data=(self.db.raw_sql(f"SELECT TICKER, AUDIT_MEMBERSHIP, CG_MEMBERSHIP, COMP_MEMBERSHIP, NOM_MEMBERSHIP FROM risk.rmdirectors WHERE YEAR BETWEEN '{self.lastyear}' AND '{self.thisyear}' AND TICKER IN {self.SP500List}")))
  ...
  def is_CEO_Dual(self):
    dataframe = pd.DataFrame(data=(self.db.raw_sql(f"SELECT TICKER, EMPLOYMENT_CEO, EMPLOYMENT_CHAIRMAN FROM risk.rmdirectors WHERE YEAR BETWEEN '{self.lastyear}' AND '{self.thisyear}' AND TICKER IN {self.SP500List}")))
  ...
  def read_in_data_from_wrds(self):
    ...
    WHERE g.YEAR BETWEEN '{self.lastyear}' AND '{self.thisyear}' AND g.TICKER IN {self.SP500List}
    '''
...
def main():
  ...
  inst.SP500List = inst.get_SP500_companies()
